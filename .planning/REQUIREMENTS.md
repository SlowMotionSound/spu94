...
- [x] **AA-01**: With Anti-Aliasing ON (default), voice playback uses 4-tap Gaussian interpolation — identical to current output, no regression
- [x] **AA-02**: With Anti-Aliasing OFF, voice playback uses zero-order hold (no interpolation) — audible aliasing artifacts from sample-skipping on pitch-up, sample-repeating on pitch-down
- [x] **AA-03**: Single global toggle on the sampler (not per-voice), visible in the Sampler window, defaults to ON on fresh state
...
| AA-01 | Phase 32 | Sampler Anti-Aliasing Toggle | Complete |
| AA-02 | Phase 32 | Sampler Anti-Aliasing Toggle | Complete |
| AA-03 | Phase 32 | Sampler Anti-Aliasing Toggle | Complete |
