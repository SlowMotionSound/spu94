...
---
*Last updated: 2026-04-23 — Phase 6 complete (ctypes binding with runtime-reflection `Register` IntEnum + import-time drift asserts, raw-panel `api.py` + `SPU94` class, strict int16 C-contiguous numpy contract, native `spu94` CLI with vendored dr_wav + jsmn + `--preset`/`--config`/`--list-presets`, `manylinux_2_28` wheel via scikit-build-core + cibuildwheel, polished 11-section README, four fuzz scripts migrated to new binding, ADR-Phase-6-A..H filed — including H for master-send default relocation into preset tables to close the `--config` override-shape silent-output trap, CR-01 Q15 input_scale fix, CLI work_buf heap-residue reset, HI-01..05 code-review fixes. 66/66 ctest green, UAT 5/5 pass, Human UAT SC-4 closed with README content polish deferred to post-M4). Requirements PYBIND-01..06, CLI-01..04, DOCS-04 validated. Next: Phase 7 — Verification (golden files, witness diff, modulation harness, LEVERS-CATALOG.md, BIBLIOGRAPHY.md).*
