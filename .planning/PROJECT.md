...
- [x] Ship Python bindings via ctypes so tests, analysis, and exploration happen in Python+numpy *(Validated in Phase 6: PYBIND-01..06 — ctypes `_binding.py` with runtime-reflection `Register` IntEnum built from `spu94_reg_name`/`spu94_reg_hw_offset` at import, import-time drift asserts on state size + reg count + preset count, strict int16 C-contiguous numpy contract on `process`/`flush`, `SPU94` class + raw-panel API, `pip install -e .` via scikit-build-core)*
- [x] Ship a small CLI (`spu94`) that processes WAV files end-to-end for witness diffs and golden-file tests *(Validated in Phase 6: CLI-01..04 — native C binary via CMake, vendored dr_wav + jsmn, `--preset`/`--config`/`--list-presets`/`--help`, one-line stderr error contract, `verify-no-drwav-in-libspu94.sh` CI gate proves dr_wav never leaks into `libspu94`)*
- [x] Build on Linux (primary dev target) *(Validated continuously since Phase 1; Phase 6 added `manylinux_2_28` wheel via cibuildwheel + `$ORIGIN` RPATH + `verify-wheel-tag.sh`)*
...
---
*Last updated: 2026-04-24 — Phase 7 complete (verification infrastructure: 50 committed goldens + reproducibility Docker container pinned to `debian:bookworm-slim@sha256:5a2a80d...`, witness-diff measurement harness against lv2-psx-reverb at pinned SHA `424e1e8...` with split-band aligned-RMS divergence (measurement-only per D-06; tolerance ADR deferred), 35-register × 3-mode modulation harness with stability + determinism gates (all 211 cases green on first run), `docs/LEVERS-CATALOG.md` with AUTO columns populated (12 free / 6 sample-quantized / 17 catastrophic) and first HAND annotations authored, hot-path allocation gate as hard CI fail via strace filter on brk/mmap/mmap2/munmap/mremap, pytest-benchmark timing harness report-only per D-20, `docs/COVERAGE.md` with CI-enforced validator mapping 77 tests to 35 registers, `docs/BIBLIOGRAPHY.md` at 20 entries with four-tier clustering + CI cross-ref gate, ADR-Phase-7-A..H landing all 22 D-XX decisions, 10/10 code-review warnings fixed in-phase. 73/73 ctest green, Human UAT 2/2 pass (host + Docker reproducibility, HAND preservation contract). Notable findings: witness-diff empirically confirms Phase-4 high-band brightness gap prediction (+30 to +52 dBFS on sine_1khz); every register but vAPF1 clean through 11 kHz under modulation. Requirements TEST-01, TEST-03, TEST-04, TEST-05, TEST-08, BUILD-06, BUILD-08, DOCS-02, DOCS-03 validated. Next: Phase 8 — MCU cross-compile + CI hardening (prove the MCU-portability claim with arm-none-eabi-gcc build + `.text` under 64 kB + no heap/pthread symbols).*
