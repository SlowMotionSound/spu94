...
1. **Register-count interpretation (vLIN/vRIN inclusion)** — see Assumption A1. Flagged as a planner-resolvable question; user confirms during discuss-checkpoint or plan-check. Recommendation: exclude (stay at 33); alternative: include (move to 35 and update all project docs).
2. **mBASE side-effect final decision** — see "mBASE Side-Effect Evidence § Recommendation for ADR-0006." Research contradicts D-09. User confirms ADR-0006 lands as "snap BufferAddress on every mBASE write," not "floor-only." Recommended: update D-09 language to strike "preliminary floor-only" and replace with "snap — see ADR-0006."
3. **Alignment guarantee for caller-provided buffer** — CONTEXT § Deferred flags this as planner-to-decide. Recommendation: commit to 16-byte alignment requirement in Phase 2 (documented in `spu94_init` doc-comment; validated at runtime). Not deferring.
4. **Thread-safety stance** — CONTEXT § Deferred flags this as Phase 2 or Phase 5. Recommendation: Phase 2 adds a one-sentence doc-comment on `spu94_init`: "A `spu94_state` is not thread-safe; concurrent access from multiple threads requires external synchronization." No implementation cost; preserves Phase 5's freedom to add more detailed guidance for API-06.
5. **mBASE side-effect seam exposure** — CONTEXT § Claude's Discretion asks whether the seam is exposed as a function pointer in Phase 2 or kept internal. Recommendation: **internal only in Phase 2** (stored as a `static const` default handler, not runtime-swappable). Controllers unpins it later by promoting the handler field to `state->mbase_handler` (runtime-swappable). Rationale: D-22 says pinned by SPU-94; exposing the function pointer now adds surface area before Controllers needs it. Logged as a deliberate decision per CONTEXT's "rationale must be logged" note.
6. **Register-name string accessor prefix convention** — CONTEXT § Claude's Discretion. Recommendation: return `"vIIR"` (not `"SPU94_REG_vIIR"`). Shorter for debug dumps and UI labels. The enum name is a C identifier; the string is human-facing.
