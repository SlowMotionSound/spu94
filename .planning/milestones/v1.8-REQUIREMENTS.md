# Requirements Archive: v1.8 PSX Voice Engine
...
- [x] **VOICE-01**: ADPCM decode from dedicated 512 KB voice RAM buffer (decode-only, no encode in hot path)
- [x] **VOICE-02**: Single-counter Gaussian interpolation per voice (bits 12+ = sample index, bits 4-11 = Gauss index)
- [x] **VOICE-03**: Pitch register per voice with hardware-mandated 0x3FFF clamp
- [x] **VOICE-04**: Per-voice unsigned L/R volume (0-32767 range, no phase inversion)
- [x] **VOICE-05**: WAV-to-ADPCM encode at sample load time (using existing encoder)
- [x] **VOICE-06**: 24 independent voice state structs with isolated Gaussian ring buffers
...
- [x] **MIX-01**: 24-voice int32 accumulator with sat_s16 at final output stage
- [x] **MIX-02**: Per-voice reverb-on flag (EON) — voice contributes to reverb send when enabled
- [x] **MIX-03**: Master Volume L/R applied after voice summation
- [x] **MIX-04**: KON/KOFF with pending-tick semantics (apply at start of next sample tick)
- [x] **MIX-05**: Dry voice sum routes to DAC section; reverb-send sum routes to existing reverb engine input
- [x] **MIX-06**: Voice engine and coloration bus (ADPCM patina) are independent — both can be active simultaneously

### SPU RAM

- [x] **RAM-01**: Dedicated 512 KB voice RAM buffer (separate from reverb work buffer — deliberate deviation from shared-RAM PS1 spec)
- [x] **RAM-02**: Dedicated 512 KB reverb RAM buffer (existing work buffer, unchanged)
- [x] **RAM-03**: Sample load validates address bounds within voice RAM
- [x] **RAM-04**: Sample addressing uses PS1-style byte offsets within the voice RAM space

### Standalone Testbed

- [x] **TEST-01**: Load WAV file into voice RAM (encode to ADPCM on load)
- [x] **TEST-02**: Trigger single voice playback from GUI (pitch control)
- [x] **TEST-03**: MIDI note input triggers voices in standalone (JUCE native MIDI)
- [x] **TEST-04**: Standalone remains the development testbed — no plugin UX changes in v1.8
...
|---|---|---|---|
| VOICE-01 | Phase 27 | Single Voice Playback | Complete |
| VOICE-02 | Phase 27 | Single Voice Playback | Complete |
| VOICE-03 | Phase 27 | Single Voice Playback | Complete |
| VOICE-04 | Phase 27 | Single Voice Playback | Complete |
| VOICE-05 | Phase 27 | Single Voice Playback | Complete |
| VOICE-06 | Phase 27 | Single Voice Playback | Complete |
| RAM-01 | Phase 27 | Single Voice Playback | Complete |
| RAM-02 | Phase 27 | Single Voice Playback | Complete |
| RAM-03 | Phase 27 | Single Voice Playback | Complete |
| RAM-04 | Phase 27 | Single Voice Playback | Complete |
...
| MIX-01 | Phase 30 | 24-Voice Polyphony + Mixer | Complete |
| MIX-02 | Phase 30 | 24-Voice Polyphony + Mixer | Complete |
| MIX-03 | Phase 30 | 24-Voice Polyphony + Mixer | Complete |
| MIX-04 | Phase 30 | 24-Voice Polyphony + Mixer | Complete |
| MIX-05 | Phase 30 | 24-Voice Polyphony + Mixer | Complete |
| MIX-06 | Phase 30 | 24-Voice Polyphony + Mixer | Complete |
| TEST-01 | Phase 31 | Standalone Testbed UX | Complete |
| TEST-02 | Phase 31 | Standalone Testbed UX | Complete |
| TEST-03 | Phase 31 | Standalone Testbed UX | Complete |
| TEST-04 | Phase 31 | Standalone Testbed UX | Complete |
