...
void spu94_process_split(spu94_state *state,
    ...
    for (uint32_t i = 0; i < num_samples; i++) {
        ...
        /* Split output: voice bus (3-bus) and reverb bus written separately.
         * No DAC — caller handles post-mix processing. */
        if (voice_L != NULL) voice_L[i] = sat_s16(
            (int32_t)q15_mul_truncate(mix_dry_l,   state->dry_fader)
          + (int32_t)q15_mul_truncate(mix_adpcm_l, state->adpcm_fader)
          + (int32_t)q15_mul_truncate(voice_dry_l, state->sampler_fader));
        if (voice_R != NULL) voice_R[i] = sat_s16(
            (int32_t)q15_mul_truncate(mix_dry_r,   state->dry_fader)
          + (int32_t)q15_mul_truncate(mix_adpcm_r, state->adpcm_fader)
          + (int32_t)q15_mul_truncate(voice_dry_r, state->sampler_fader));
        if (reverb_L != NULL) reverb_L[i] = q15_mul_truncate(rev_l, state->reverb_fader);
        if (reverb_R != NULL) reverb_R[i] = q15_mul_truncate(rev_r, state->reverb_fader);
    }
}
