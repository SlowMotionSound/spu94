                                        ...
                                        juce::MidiBuffer& midiMessages)
{
    ...
    if (isStandalone)
    {
        ...
        //
        // ORDERING (apply runs BEFORE the Phase 46 sidechain-duck block ~590 lines below):
        // base_vol is the sweep/duck CEILING the C-core tick consumes on the same tick
        // (spu94_voice.c STEP 0: vol = sweep.level * base_vol >> 15, or vol = base_vol when
        // no sweep). Running the fan-out HERE -- before MIDI dispatch and before the duck --
        // means the sweep reads the fresh Level-scaled ceiling, and the loop writes ONLY
        // base_vol_l/r + set_non/set_pmon, never any sweep field, so an in-flight tremolo
        // sweep survives the re-base intact (proven by voice_controls_sweep_interaction).
        //
        // CAVEAT (untested, tracked as TODO WR-02): the Phase 46 duck snapshots duckOrigLevel
        // ONCE at duck-start, but this loop rewrites base_vol every block. Moving the Level
        // fader WHILE a voice is mid-duck can desync the duck's recovery target from the moved
        // ceiling. There is no "move Level mid-duck" test yet, so this interaction is a known
        // follow-up (61-RESEARCH.md Pitfall 4 / Open Question 2), NOT a verified-clean path.
